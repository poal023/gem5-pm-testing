...
class MI300XAccelwattchALUPower(AccelwattchBasePowerModel):
    def __init__(self, gpu: Shader, act_energies, scaling_factors, interval):
        ...
        self.name = "MI300XAccelwattchALUPower"
    ...
    def ialu_energy(self) -> float:
        ...
        return ialu_accesses * self._act_energies["ALU"]

    def ialu_accesses(self) -> float:
        return (
            sum(
                self.get_stat(f"CUs{i}.intInsts") for i in range(self._num_cus)
            )
