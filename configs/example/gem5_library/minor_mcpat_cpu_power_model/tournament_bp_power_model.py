...
class TournamentBPPower(McPATPowerModel):
    def __init__(self, branch_pred: TournamentBP, xml_tree):
        ...
        self.name = "BP"
        self._btb_read_ae = self.obtain_act_energy("BTB", "Read")
        self._btb_write_ae = self.obtain_act_energy("BTB", "Write")

        self._glob_pred_read_ae = self.obtain_act_energy("GlobalPred", "Read")
        self._glob_pred_write_ae = self.obtain_act_energy(
            "GlobalPred", "Write"
        )

        self._l1_local_pred_read_ae = self.obtain_act_energy("L1Pred", "Read")
        self._l1_local_pred_write_ae = self.obtain_act_energy(
            "L1Pred", "Write"
        )

        self._l2_local_pred_read_ae = self.obtain_act_energy("L2Pred", "Read")
        self._l2_local_pred_write_ae = self.obtain_act_energy(
            "L2Pred", "Write"
        )

        self._chooser_pred_read_ae = self.obtain_act_energy(
            "PredChooser", "Read"
        )
        self._chooser_pred_write_ae = self.obtain_act_energy(
            "PredChooser", "Write"
        )

        self._ras_pred_read_ae = self.obtain_act_energy("RAS", "Read")
        self._ras_pred_write_ae = self.obtain_act_energy("RAS", "Write")
